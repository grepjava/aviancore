/* The shared response cache, tested against avian_cache.c directly:
 *
...
extern void (*av_cache_test_after_claim)(void);
...
static struct hit get(const char *key, uint64_t now) {
    ...
    uint32_t head_len = 0;
    uint32_t body_len = 0;
    r.found = av_cache_get((const uint8_t *)key, strlen(key), now, out, sizeof out,
...
static int put_sized(const char *key, uint64_t sequence, uint64_t now, uint64_t ttl,
    ...
    uint64_t target = av_cache_target_hash((const uint8_t *)key, strlen(key));
    return av_cache_put((const uint8_t *)key, strlen(key), target, sequence, now, 0, ttl,
...
static uint64_t slot_bytes(uint32_t max_head, uint32_t max_body) {
    return (uint64_t)AV_CACHE_MAX_KEY + max_head + max_body + 200;
}
...
static void replace_while_paused(void) {
    ...
    uint64_t newer = av_cache_begin();
    CHECK(put("/paused", newer, 5000, 60000, 201, "NEW") == 1);
}

static void test_paused_writer(void) {
    CHECK(av_cache_init(1024 * KIB, 1 * KIB, 4 * KIB) > 0);
    uint64_t older = av_cache_begin();
    av_cache_test_after_claim = replace_while_paused;
    put("/paused", older, 1000, 60000, 200, "OLD");
    av_cache_test_after_claim = NULL;
...
static void write_while_claimed(void) {
    ...
    CHECK(put("/g", av_cache_begin(), 0, 45000, 200, "g") == 1);
}

static void test_dead_writer(void) {
    /* One size class of exactly PROBES slots, so every entry competes for
     * every slot. */
    CHECK(av_cache_init(4 * slot_bytes(1 * KIB, 4 * KIB), 1 * KIB, 4 * KIB) == 4);
    CHECK(put("/a", av_cache_begin(), 0, 60000, 200, "a") == 1);
    CHECK(put("/b", av_cache_begin(), 0, 50000, 200, "b") == 1);
    CHECK(put("/c", av_cache_begin(), 0, 40000, 200, "c") == 1);

    pid_t child = fork();
    if (child == 0) {
        av_cache_test_after_claim = die_holding_slot;
        put("/d", av_cache_begin(), 0, 60000, 200, "d");
        _exit(1);
    }
    int wstatus = 0;
    ...
    CHECK(put("/e", av_cache_begin(), 0, 60000, 200, "e") == 1);
    CHECK(get("/a", 1).found);
    CHECK(get("/b", 1).found);
    CHECK(get("/c", 1).found);
    ...
    av_cache_test_after_claim = write_while_claimed;
    CHECK(put("/f", av_cache_begin(), 0, 60000, 200, "f") == 1);
    av_cache_test_after_claim = NULL;
    /* Once for this writer, once for the one it started from the hook. */
    CHECK(hook_calls == 2);
...
static void store_large_older(void) {
    if (hook_calls++ > 0) return;
    /* Number 1 is below every request av_cache_begin has numbered. */
    CHECK(put_sized("/race", 1, 0, 60000, 200, large_body, sizeof large_body) == 1);
}

static void store_small_newer(void) {
    if (hook_calls++ > 0) return;
    CHECK(put_sized("/race2", av_cache_begin(), 0, 1000, 200, small_body,
                    sizeof small_body) == 1);
}

static void test_size_classes(void) {
    CHECK(av_cache_init(8 * KIB * KIB, 1 * KIB, 128 * KIB) > 0);
    ...
    CHECK(put_sized("/grow", av_cache_begin(), 0, 60000, 200, small_body, sizeof small_body) == 1);
    CHECK(put_sized("/grow", av_cache_begin(), 0, 60000, 200, large_body, sizeof large_body) == 1);
    CHECK(get("/grow", 1).body_len == sizeof large_body);

    /* An older large response does not come back when a newer small one
     * expires. */
    CHECK(put_sized("/shrink", av_cache_begin(), 0, 60000, 200, large_body, sizeof large_body) == 1);
    CHECK(put_sized("/shrink", av_cache_begin(), 0, 1000, 200, small_body, sizeof small_body) == 1);
     ...
     * kept. */
    uint64_t earlier = av_cache_begin();
    uint64_t later = av_cache_begin();
    ...
    hook_calls = 0;
    av_cache_test_after_claim = store_large_older;
    CHECK(put_sized("/race", av_cache_begin(), 0, 1000, 200, small_body,
                    sizeof small_body) == 1);
    av_cache_test_after_claim = NULL;
    ...
    hook_calls = 0;
    av_cache_test_after_claim = store_small_newer;
    CHECK(put_sized("/race2", av_cache_begin(), 0, 60000, 200, large_body,
                    sizeof large_body) == 1);
    av_cache_test_after_claim = NULL;
...
static void test_invalidation(void) {
    CHECK(av_cache_init(1024 * KIB, 1 * KIB, 4 * KIB) > 0);
    uint64_t before = av_cache_begin();
    CHECK(put("/item", before, 0, 60000, 200, "before") == 1);
    char others[8][32];
    for (int i = 0; i < 8; i++) {
        snprintf(others[i], sizeof others[i], "/other%d", i);
        CHECK(put(others[i], av_cache_begin(), 0, 60000, 200, "other") == 1);
    }
    CHECK(get("/item", 1).found);

    av_cache_invalidate(av_cache_target_hash((const uint8_t *)"/item", 5));
    ...
    CHECK(put("/item", av_cache_begin(), 5, 60000, 200, "after") == 1);
...
static void test_age_and_flush(void) {
    CHECK(av_cache_init(1024 * KIB, 1 * KIB, 4 * KIB) > 0);
    uint64_t target = av_cache_target_hash((const uint8_t *)"/aged", 5);
    CHECK(av_cache_put((const uint8_t *)"/aged", 5, target, av_cache_begin(), 1000, 30000,
    ...
    CHECK(put("/flushed", av_cache_begin(), 0, 60000, 200, "x") == 1);
...
static void store_newer_small_while_large_writes(void) {
    if (hook_calls++ > 0) return;
    CHECK(put_sized("/evicted", av_cache_begin(), 0, 30000, 200, small_body, 100) == 1);
}
...
static int replace_small_copy(const char *key) {
    ...
    for (int i = 0; i < 500; i++) {
        ...
        put_sized(filler, av_cache_begin(), 0, 60000, 200, small_body, 100);
...
static void store_newer_and_replace_it(void) {
    if (hook_calls++ > 0) return;
    CHECK(put_sized("/gone", av_cache_begin(), 0, 30000, 200, small_body, 100) == 1);
...
static void test_eviction(void) {
     ...
     * to 16 KiB. */
    CHECK(av_cache_init(2 * 8 * (8 * KIB + 200), 1 * KIB, 16 * KIB) > 0);
    ...
    uint64_t older = av_cache_begin();
    hook_calls = 0;
    av_cache_test_after_claim = store_newer_small_while_large_writes;
     ...
     * is still not served. */
    hook_calls = 0;
    av_cache_test_after_claim = store_newer_and_replace_it;
    CHECK(put_sized("/gone", av_cache_begin(), 0, 60000, 200, large_body, 10 * KIB) == 0);
    av_cache_test_after_claim = NULL;
    CHECK(hook_calls >= 2);
    ...
    uint64_t earlier = av_cache_begin();
    CHECK(put_sized("/late", av_cache_begin(), 0, 30000, 200, small_body, 100) == 1);
    ...
    CHECK(put_sized("/late", av_cache_begin(), 0, 60000, 200, large_body, 10 * KIB) == 1);
...
static void test_space(void) {
    ...
    CHECK(av_cache_init(2 * 4 * (8 * KIB + 200), 1 * KIB, 16 * KIB) > 0);

    /* The small copy of /k outlives everything else, so nothing would replace
     * it for being near its end. */
    CHECK(put_sized("/k", av_cache_begin(), 0, 120000, 200, small_body, 100) == 1);
    CHECK(put_sized("/s1", av_cache_begin(), 0, 60000, 200, small_body, 100) == 1);
    CHECK(put_sized("/s2", av_cache_begin(), 0, 60000, 200, small_body, 100) == 1);
    CHECK(put_sized("/s3", av_cache_begin(), 0, 60000, 200, small_body, 100) == 1);

    /* A newer, large /k retires the small one, and the next small entry takes
     * that slot rather than a live entry's. */
    CHECK(put_sized("/k", av_cache_begin(), 0, 60000, 200, large_body, 10 * KIB) == 1);
    CHECK(put_sized("/s4", av_cache_begin(), 0, 90000, 200, small_body, 100) == 1);
    ...
    uint64_t before = av_cache_begin();
    av_cache_invalidate(av_cache_target_hash((const uint8_t *)"/x", 2));
