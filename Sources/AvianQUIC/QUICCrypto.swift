...
public enum QUICCipher: UInt16 {
    ...
    public var aead: Int32 {
        switch self {
        case .aes128GCMSHA256: return Int32(AV_AEAD_AES128GCM)
        case .aes256GCMSHA384: return Int32(AV_AEAD_AES256GCM)
        case .chacha20Poly1305SHA256: return Int32(AV_AEAD_CHACHA20POLY1305)
        }
    }

    @inlinable
    public var hash: Int32 {
        self == .aes256GCMSHA384 ? Int32(AV_SHA384) : Int32(AV_SHA256)
    }
...
public func quicExpandLabel(hash: Int32,
    ...
    return info.withUnsafeBufferPointer { buf in
        av_hkdf_expand(hash, secret, secretLen, buf.baseAddress, n, out, outLen) == 0
    }
}
...
public struct QUICKeys {
    ...
    public static func derive(secret: UnsafePointer<UInt8>, secretLen: Int,
        ...
        keys.aead = key.withUnsafeBufferPointer { av_aead_new(cipher.aead, $0.baseAddress) }
...
public struct QUICHeaderKey {
    ...
    public static func derive(secret: UnsafePointer<UInt8>, secretLen: Int,
        ...
        out.hp = key.withUnsafeBufferPointer { av_hp_new(cipher.aead, $0.baseAddress) }
    ...
    public func mask(_ sample: UnsafePointer<UInt8>,
        ...
        return av_hp_mask(hp, sample, out) == 0
    }

    public mutating func destroy() {
        if let hp { av_hp_free(hp) }
        hp = nil
    }
}
    ...
    let extracted = destinationCID.withBytes { cid, len in
        salt.withUnsafeBufferPointer { s in
            initialSecret.withUnsafeMutableBufferPointer { out in
                av_hkdf_extract(Int32(AV_SHA256), s.baseAddress, s.count,
    ...
    let ok = initialSecret.withUnsafeBufferPointer { prk in
        clientSecret.withUnsafeMutableBufferPointer { c in
            serverSecret.withUnsafeMutableBufferPointer { s in
                quicExpandLabel(hash: Int32(AV_SHA256), secret: prk.baseAddress!, secretLen: 32,
...
public enum QUICProtect {
    ...
    public static func seal(_ packet: UnsafeMutablePointer<UInt8>,
        ...
        let sealed: Int = withUnsafeMutableBytes(of: &nonce) { raw in
            ...
            return Int(av_aead_seal(aead, n, packet, headerLength,
    ...
    public static func openBody(_ packet: UnsafeMutablePointer<UInt8>,
        ...
        let plaintextLength: Int = withUnsafeMutableBytes(of: &nonce) { raw in
            ...
            return Int(av_aead_open(aead, n, packet, un.headerLength,
