...
public enum HTTPResponseWriter {
    ...
    /// Status line where the application supplied its own reason phrase, as
    /// one opaque string such as `"200 OK"`.
    @inlinable
    ...
    /// negotiate. `serverName` is the value of the `Server` header. `omitBody`
    /// is for a HEAD request: the length stays, the reason phrase does not go
    /// out.
    public static func writeError(_ buf: inout ByteBuffer, status: Int,
                                  closeConnection: Bool, dateCache: borrowing DateCache,
                                  serverName: StaticString, omitBody: Bool = false) {
