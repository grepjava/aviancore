/* permessage-deflate: see avian_wsdeflate.h. */

#include "avian_wsdeflate.h"
...
void *av_ws_deflate_new(int window_bits, int mem_level) {
...
void *av_ws_inflate_new(int window_bits) {
...
void av_ws_deflate_free(void *z) {
    if (!z) return;
    deflateEnd((z_stream *)z);
    free(z);
}

void av_ws_inflate_free(void *z) {
    if (!z) return;
    inflateEnd((z_stream *)z);
    free(z);
}

int av_ws_deflate_reset(void *z) { return deflateReset((z_stream *)z) == Z_OK ? 0 : -1; }
int av_ws_inflate_reset(void *z) { return inflateReset((z_stream *)z) == Z_OK ? 0 : -1; }

int av_ws_deflate_run(void *zp, const uint8_t *in, size_t n,
...
int av_ws_inflate_run(void *zp, const uint8_t *in, size_t n,
                      uint8_t *out, size_t cap, size_t *consumed, size_t *produced) {
    z_stream *z = (z_stream *)zp;
    static const uint8_t nothing = 0;
    if (n > UINT_MAX) n = UINT_MAX;
