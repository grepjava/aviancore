/* The OpenSSL half of the ACME client. See avian_acme.h. */
#define _GNU_SOURCE

#include "avian_acme.h"
...
int av_acme_b64url(const uint8_t *in, size_t n, char *out, size_t cap) {
...
int av_acme_mkdirs(const char *path) {
    char buf[4096];
...
int av_acme_write_file(const char *path, const uint8_t *data, size_t n, int mode) {
    char tmp[4200];
...
int av_acme_rename(const char *from, const char *to) { return rename(from, to); }

int av_acme_exit_ok(int status) { return WIFEXITED(status) && WEXITSTATUS(status) == 0; }
...
av_acme_key *av_acme_key_load_or_create(const char *p, char *e, size_t l) {
    (void)p; if (e && l) snprintf(e, l, "built without OpenSSL"); return NULL;
}
void av_acme_key_free(av_acme_key *k) { (void)k; }
int av_acme_jwk(av_acme_key *k, char *o, size_t c) { (void)k; (void)o; (void)c; return -1; }
int av_acme_thumbprint(av_acme_key *k, char *o, size_t c) { (void)k; (void)o; (void)c; return -1; }
int av_acme_sign(av_acme_key *k, const uint8_t *d, size_t n, char *o, size_t c) {
    (void)k; (void)d; (void)n; (void)o; (void)c; return -1;
}
int av_acme_csr(const char *n, const char *k, char *o, size_t c, char *e, size_t l) {
    (void)n; (void)k; (void)o; (void)c; (void)e; (void)l; return -1;
}
int av_acme_alpn_cert(const char *n, const char *a, const char *c, const char *k, char *e, size_t l) {
    (void)n; (void)a; (void)c; (void)k; (void)e; (void)l; return -1;
}
int av_acme_placeholder(const char *n, const char *c, const char *k, char *e, size_t l) {
    (void)n; (void)c; (void)k; (void)e; (void)l; return -1;
}
int av_acme_needs_certificate(const char *p, const char *n, long s) { (void)p; (void)n; (void)s; return 1; }
av_acme_resp *av_acme_https(const char *m, const char *u, const char *t, const uint8_t *b,
...
int av_acme_resp_status(av_acme_resp *r) { (void)r; return 0; }
int av_acme_resp_header(av_acme_resp *r, const char *n, char *o, size_t c) {
    (void)r; (void)n; (void)o; (void)c; return -1;
}
const uint8_t *av_acme_resp_body(av_acme_resp *r, size_t *len) { (void)r; *len = 0; return NULL; }
void av_acme_resp_free(av_acme_resp *r) { (void)r; }
...
#define PLACEHOLDER_ORG "avian acme placeholder"
...
static int write_key(EVP_PKEY *pkey, const char *path) {
    ...
    if (ok) ok = av_acme_write_file(path, (const uint8_t *)data, (size_t)n, 0600) == 0;
    BIO_free(bio);
    return ok ? 0 : -1;
}
...
static int write_cert(X509 *cert, const char *path) {
    ...
    char *data = NULL;
    long n = BIO_get_mem_data(bio, &data);
    if (ok) ok = av_acme_write_file(path, (const uint8_t *)data, (size_t)n, 0644) == 0;
    BIO_free(bio);
    return ok ? 0 : -1;
}

av_acme_key *av_acme_key_load_or_create(const char *path, char *err, size_t err_len) {
    EVP_PKEY *pkey = NULL;
    FILE *f = fopen(path, "r");
    if (f) {
    ...
    struct av_acme_key *key = calloc(1, sizeof *key);
    if (!key) { EVP_PKEY_free(pkey); return NULL; }
    key->pkey = pkey;
    return key;
}

void av_acme_key_free(av_acme_key *key) {
    if (!key) return;
    EVP_PKEY_free(key->pkey);
    free(key);
}

int av_acme_jwk(av_acme_key *key, char *out, size_t cap) {
    ...
    int ok = BN_bn2binpad(x, xb, 32) == 32 && BN_bn2binpad(y, yb, 32) == 32
        && av_acme_b64url(xb, 32, xs, sizeof xs) > 0
        && av_acme_b64url(yb, 32, ys, sizeof ys) > 0;
    BN_free(x);
    BN_free(y);
    if (!ok) return -1;
...
int av_acme_thumbprint(av_acme_key *key, char *out, size_t cap) {
    char jwk[256];
    int n = av_acme_jwk(key, jwk, sizeof jwk);
    if (n < 0) return -1;
    uint8_t md[32];
    unsigned int mdlen = 0;
    if (!EVP_Digest(jwk, (size_t)n, md, &mdlen, EVP_sha256(), NULL)) return -1;
    return av_acme_b64url(md, mdlen, out, cap);
}

int av_acme_sign(av_acme_key *key, const uint8_t *data, size_t n, char *out, size_t cap) {
    ...
    ECDSA_SIG_free(sig);
    if (!ok) return -1;
    return av_acme_b64url(raw, 64, out, cap);
}
...
int av_acme_csr(const char *names, const char *key_path, char *out, size_t cap,
    ...
    result = av_acme_b64url(der, (size_t)n, out, cap);
...
int av_acme_alpn_cert(const char *name, const char *key_authorization,
...
int av_acme_placeholder(const char *names, const char *cert_path, const char *key_path,
                        char *err, size_t err_len) {
    char sans[8192];
    if (san_list(names, sans, sizeof sans) != 0) {
        if (err && err_len) snprintf(err, err_len, "no names for the certificate");
...
int av_acme_needs_certificate(const char *path, const char *names, long renew_seconds) {
    FILE *f = fopen(path, "r");
    if (!f) return 1;
...
av_acme_resp *av_acme_https(const char *method, const char *url,
...

    {
        ...
        char te[64];
        char cl[32];
        if (av_acme_resp_header(resp, "transfer-encoding", te, sizeof te) > 0
            ...
            if (dechunk(resp->body, resp->body_len, &decoded) != 0) {
                ...
                av_acme_resp_free(resp);
                resp = NULL;
                goto fail;
            }
            resp->body_len = decoded;
        } else if (av_acme_resp_header(resp, "content-length", cl, sizeof cl) > 0) {
...
int av_acme_resp_status(av_acme_resp *resp) { return resp ? resp->status : 0; }

int av_acme_resp_header(av_acme_resp *resp, const char *name, char *out, size_t cap) {
    if (!resp) return -1;
...
const uint8_t *av_acme_resp_body(av_acme_resp *resp, size_t *len) {
    if (!resp) { *len = 0; return NULL; }
    *len = resp->body_len;
    return resp->body;
}

void av_acme_resp_free(av_acme_resp *resp) {
    if (!resp) return;
    free(resp->raw);
    free(resp);
}
