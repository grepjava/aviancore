...
int av_poll_create(void) { return epoll_create1(EPOLL_CLOEXEC); }

static uint32_t to_epoll(uint32_t mask) {
    uint32_t e = EPOLLRDHUP;   /* peer half-close must be visible, not silent */
    if (mask & AV_POLL_READ)  e |= EPOLLIN;
    if (mask & AV_POLL_WRITE) e |= EPOLLOUT;
    return e;
}
...
int av_poll_wait(int pfd, av_event *out, int max_events, int timeout_ms) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (e & EPOLLIN)  m |= AV_POLL_READ;
        if (e & EPOLLOUT) m |= AV_POLL_WRITE;
        if (e & EPOLLERR) m |= AV_POLL_ERR;
        if (e & (EPOLLHUP | EPOLLRDHUP)) m |= AV_POLL_HUP;
...
static int kq_apply(int pfd, int fd, uint32_t mask, uint64_t token) {
    ...
    EV_SET(&ch[n], fd, EVFILT_READ,
           (mask & AV_POLL_READ) ? (EV_ADD | EV_ENABLE) : EV_DELETE, 0, 0, ud);
    n++;
    EV_SET(&ch[n], fd, EVFILT_WRITE,
           (mask & AV_POLL_WRITE) ? (EV_ADD | EV_ENABLE) : EV_DELETE, 0, 0, ud);
    n++;
...
int av_poll_wait(int pfd, av_event *out, int max_events, int timeout_ms) {
    ...
    for (int i = 0; i < n; i++) {
        uint32_t m = 0;
        if (evs[i].filter == EVFILT_READ)  m |= AV_POLL_READ;
        if (evs[i].filter == EVFILT_WRITE) m |= AV_POLL_WRITE;
        if (evs[i].flags & EV_EOF)   m |= AV_POLL_HUP;
        if (evs[i].flags & EV_ERROR) m |= AV_POLL_ERR;
...
int av_shutdown_write(int fd) { return shutdown(fd, SHUT_WR); }
int av_close(int fd) { return close(fd); }
...
ssize_t av_read(int fd, void *buf, size_t n)         { return read(fd, buf, n); }
ssize_t av_write(int fd, const void *buf, size_t n)  { return write(fd, buf, n); }
ssize_t av_writev(int fd, const struct iovec *iov, int iovcnt) { return writev(fd, iov, iovcnt); }
...
static int static_open_beneath(const char *root, const char *relative) {
    if (g_openat2_unavailable < 0) {
        const char *env = getenv("AVIAN_NO_OPENAT2");
...
int av_connect_udp(const char *host, uint16_t port) {
    ...
    /* The same parse-and-return as av_connect_tcp: a nameserver address comes
...
int av_errno(void) { return errno; }
void av_set_errno(int e) { errno = e; }
const char *av_strerror(int e) { return strerror(e); }
int av_err_is_again(int e) { return e == EAGAIN || e == EWOULDBLOCK; }
int av_err_is_intr(int e) { return e == EINTR; }
...
int av_http_date(char *buf29, int64_t t) {
...
int av_signal_pipe_init(void) {
    if (g_sigpipe[0] >= 0) return g_sigpipe[0];
    if (pipe(g_sigpipe) != 0) return -1;
    av_set_nonblock(g_sigpipe[0]);
    av_set_nonblock(g_sigpipe[1]);
    av_set_cloexec(g_sigpipe[0]);
    av_set_cloexec(g_sigpipe[1]);
    install(SIGINT);
    install(SIGTERM);
    install(SIGQUIT);
    install(SIGHUP);
...
void av_ignore_sigpipe(void) { signal(SIGPIPE, SIG_IGN); }

pid_t av_fork(void) { return fork(); }
pid_t av_waitpid(pid_t pid, int *status, int nohang) {
    return waitpid(pid, status, nohang ? WNOHANG : 0);
}
int av_kill(pid_t pid, int sig) { return kill(pid, sig); }
pid_t av_getpid(void) { return getpid(); }
 ...
 * has it already; setting it keeps `pkill <name>` working when the file was
