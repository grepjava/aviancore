...
typedef struct av_acme_key av_acme_key;
typedef struct av_acme_resp av_acme_resp;
...
av_acme_resp *av_acme_https(const char *method, const char *url,
                            ...
                            char *err, size_t err_len);
int av_acme_resp_status(av_acme_resp *resp);
...
int av_acme_rename(const char *from, const char *to);
/* 1 when a wait status says the child exited with 0. */
int av_acme_exit_ok(int status);
