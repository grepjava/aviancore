/* The shared response cache. See avian_cache.h. */
#define _GNU_SOURCE
...
void (*av_cache_test_after_claim)(void) = NULL;
#define AFTER_CLAIM() do { if (av_cache_test_after_claim) av_cache_test_after_claim(); } while (0)
#else
#define AFTER_CLAIM() ((void)0)
#endif

struct av_cache_table {
    _Atomic uint64_t generation;
    _Atomic uint64_t sequence;
};

struct av_cache_slot {
...
static struct av_cache_table *g_table = NULL;
static _Atomic uint64_t *g_marks = NULL;
static uint64_t g_mark_mask = 0;
static struct av_cache_class g_classes[MAX_CLASSES];
...
static struct av_cache_slot *slot_at(const struct av_cache_class *c, uint64_t index) {
    return (struct av_cache_slot *)(c->slots + (index & c->mask) * c->slot_size);
}

static uint8_t *slot_data(struct av_cache_slot *s) {
    return (uint8_t *)(s + 1);
}
...
long av_cache_init(uint64_t total_bytes, uint32_t max_head, uint32_t max_body) {
    if (g_table) {
        long slots = 0;
        for (int i = 0; i < g_class_count; i++) slots += (long)(g_classes[i].mask + 1);
        return slots;
    }
    size_t largest = (size_t)AV_CACHE_MAX_KEY + max_head + max_body;
    ...
    size_t bytes = sizeof(struct av_cache_table);
    for (int i = 0; i < count; i++) {
        size_t slot = round_up(sizeof(struct av_cache_slot) + capacities[i]);
    ...
    g_table = (struct av_cache_table *)p;
    atomic_store(&g_table->generation, 1);
    atomic_store(&g_table->sequence, 1);
    uint8_t *at = (uint8_t *)p + sizeof(struct av_cache_table);
    ...
    for (int i = 0; i < count; i++) {
        ...
        g_classes[i].slot_size = round_up(sizeof(struct av_cache_slot) + capacities[i]);
...
int av_cache_enabled(void) { return g_table != NULL; }
uint32_t av_cache_max_head(void) { return g_max_head; }
uint32_t av_cache_max_body(void) { return g_max_body; }

void av_cache_flush(void) {
    if (g_table) atomic_fetch_add(&g_table->generation, 1);
}

uint64_t av_cache_begin(void) {
    if (!g_table) return 0;
    return atomic_fetch_add(&g_table->sequence, 1) + 1;
}

uint64_t av_cache_target_hash(const uint8_t *target, size_t target_len) {
    if (!g_table) return 0;
    return hash_key(target, target_len);
}

void av_cache_invalidate(uint64_t target_hash) {
    if (!g_table) return;
    raise_mark(target_hash, av_cache_begin());
}

/* Whether a slot holds `key` under `generation`, read without owning it: the
 * caller confirms the version word did not move. */
static int holds(const struct av_cache_class *cls, struct av_cache_slot *s, uint64_t h,
...
static int holds_newer(uint64_t h, const uint8_t *key, size_t key_len,
    ...
    for (int k = 0; k < g_class_count; k++) {
        const struct av_cache_class *cls = &g_classes[k];
        for (int p = 0; p < PROBES; p++) {
            struct av_cache_slot *s = slot_at(cls, h + (uint64_t)p);
...
static void retire_older(uint64_t h, const uint8_t *key, size_t key_len,
                         uint64_t generation, uint64_t sequence,
                         const struct av_cache_slot *keep, uint64_t self) {
    for (int k = 0; k < g_class_count; k++) {
        const struct av_cache_class *cls = &g_classes[k];
        for (int p = 0; p < PROBES; p++) {
            struct av_cache_slot *s = slot_at(cls, h + (uint64_t)p);
...
int av_cache_get(const uint8_t *key, size_t key_len, uint64_t now_ms,
                 uint8_t *out, size_t out_capacity,
                 uint32_t *head_len, uint32_t *body_len, uint16_t *status,
                 uint64_t *age_ms, uint64_t *ttl_ms) {
    if (!g_table || key_len == 0 || key_len > AV_CACHE_MAX_KEY) return 0;
     ...
     * counts. */
    struct av_cache_slot *best = NULL;
    const struct av_cache_class *best_class = NULL;
...
int av_cache_put(const uint8_t *key, size_t key_len, uint64_t target_hash,
                 uint64_t sequence, uint64_t now_ms, uint64_t age_ms, uint64_t ttl_ms,
                 uint16_t status,
                 const uint8_t *head, size_t head_len,
                 const uint8_t *body, size_t body_len) {
    if (!g_table || key_len == 0 || key_len > AV_CACHE_MAX_KEY || sequence == 0) return 0;
    ...
    const struct av_cache_class *cls = NULL;
    for (int k = 0; k < g_class_count; k++) {
    ...
    struct av_cache_slot *choice = NULL;
    ...
    uint8_t evicted_key[AV_CACHE_MAX_KEY];
    ...
    if (!(choice_version & 1) && choice->generation == generation) {
        ...
        if (klen == 0 || klen > AV_CACHE_MAX_KEY || klen > cls->capacity) klen = 0;
