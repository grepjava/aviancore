/* The shared rate-limit table. See avian_ratelimit.h. */
#define _GNU_SOURCE

#include "avian_ratelimit.h"
...
static struct av_rl_entry *g_table = NULL;
...
int av_ratelimit_init(uint64_t emission_us, uint64_t tolerance_us, int log2_entries) {
    if (g_table) return 0;
    ...
    void *p = mmap(NULL, count * sizeof(struct av_rl_entry), PROT_READ | PROT_WRITE,
                   MAP_SHARED | MAP_ANONYMOUS, -1, 0);
    if (p == MAP_FAILED) return -1;
    memset(p, 0, count * sizeof(struct av_rl_entry));
    ...
    g_table = (struct av_rl_entry *)p;
    return 0;
}

int av_ratelimit_enabled(void) { return g_table != NULL; }
...
static struct av_rl_entry *find(uint64_t key, uint64_t now) {
    ...
    for (int p = 0; p < PROBES; p++) {
        struct av_rl_entry *e = &g_table[(home + (uint64_t)p) & g_mask];
...
uint64_t av_ratelimit_check(const uint8_t *addr, size_t len, uint64_t now_us) {
    if (!g_table || !addr) return 0;
    uint64_t key;
    if (key_of(addr, len, &key) != 0) return 0;
    struct av_rl_entry *e = find(key, now_us);
    if (!e) return 0;
