...
int av_unlink(const char *path) { return unlink(path); }

const char *av_getenv(const char *name) { return getenv(name); }
...
int av_pipe(int fds[2]) {
    ...
    for (int i = 0; i < 2; i++) {
        av_set_nonblock(fds[i]);
        av_set_cloexec(fds[i]);
    }
    return 0;
}
...
struct av_mutex { pthread_mutex_t m; };
struct av_cond  { pthread_cond_t c; };

av_mutex *av_mutex_new(void) {
    av_mutex *m = (av_mutex *)malloc(sizeof *m);
    if (!m) return NULL;
...
void av_mutex_lock(av_mutex *m)   { pthread_mutex_lock(&m->m); }
void av_mutex_unlock(av_mutex *m) { pthread_mutex_unlock(&m->m); }

av_cond *av_cond_new(void) {
    av_cond *c = (av_cond *)malloc(sizeof *c);
    if (!c) return NULL;
...
void av_cond_wait(av_cond *c, av_mutex *m) { pthread_cond_wait(&c->c, &m->m); }
void av_cond_signal(av_cond *c)            { pthread_cond_signal(&c->c); }
void av_cond_broadcast(av_cond *c)         { pthread_cond_broadcast(&c->c); }
...
struct av_thread { pthread_t tid; };

av_thread *av_thread_start(void (*fn)(void *), void *arg) {
    av_thread *t = (av_thread *)malloc(sizeof *t);
    if (!t) return NULL;
    struct thread_start *s = (struct thread_start *)malloc(sizeof *s);
    ...
    pthread_attr_t attr;
    pthread_attr_init(&attr);
    /* A long-lived thread runs a whole application, not one request frame;
     * it gets the same 8 MiB a main thread would have. */
...
/* The current Worker. See the comment in avian_sys.h.
 *
...
void *av_worker_current(void) { return g_current_worker; }
void av_worker_set_current(void *worker) { g_current_worker = worker; }
...
void av_cancel_exit_timer(void) { alarm(0); }
