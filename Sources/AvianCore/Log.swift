...
public enum Log {
    ...
    public static func emit(_ l: LogLevel, _ body: (inout LogLine) -> Void) {
        ...
        withUnsafeTemporaryAllocation(of: UInt8.self, capacity: capacity) { raw in
            ...
            line.len &+= 1
            _ = av_write(2, line.buf, line.len)
        }
    }
    ...
    /// Writes an arbitrary blob (a traceback from an embedded runtime) straight through.
